...
Public Class AsigCDec
        ...
        Try
            ...
            For i = 0 To StrucAsigTer.Length - 1
                If StrucAsigTer(i).CLD_COD <> "" Then
                    ...
                    querystring += "VALUES('" + TEDE_TNIT + "','" + StrucAsigTer(i).CLD_COD + "',to_date('" + StrucAsigTer(i).FECHA_INI + "','dd/mm/yyyy'), SYSDATE,'" + Me.usuario + "',USER, SYSDATE, SYSDATE)"
                    Me.Msg += querystring
                    CrearComando(querystring)
                    na += EjecutarComando()
                End If
    ...
    Public Function COnsultarAsig(ByVal TEDE_TNIT As String) As DataTable
        ...
        If Not String.IsNullOrEmpty(TEDE_TNIT) Then
            querystring = "SELECT cld_cod,cld_nom, TEDE_FINI, case When tede_cdec is not null then '1' else '0' end Cld_Est FROM clase_dec c  Left Join(Select * From ter_dec Where tede_tnit=:TEDE_TNIT) t On t.tede_cdec=c.cld_cod  order by c.CLD_COD"
            CrearComando(querystring)
        ...
        Try
            ...
            For i = 0 To StrucAsigTer.Length - 1
                If StrucAsigTer(i).CLD_COD <> "" Then
                    ...
                    StrSql2 += " SELECT  TER_NIT, '" + StrucAsigTer(i).CLD_COD + "' , '" + StrucAsigTer(i).FECHA_INI + "', SYSDATE FROM TERCEROS WHERE TER_TIP=:TER_TIP "
