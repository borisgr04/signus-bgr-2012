...
Public Class Util
    ...
    Public Shared Function ImageByte(ByRef oFile As FileUpload) As Byte()
        If oFile.HasFile Then
            Dim reader As BinaryReader = New BinaryReader(oFile.PostedFile.InputStream)
            Dim image As Byte() = reader.ReadBytes(oFile.PostedFile.ContentLength)
            Return image
        End If
    End Function

    Public Shared Function ArchivoTemporal(ByVal extension As String) As String
        Dim oPath As String = Path.ChangeExtension(Path.GetTempFileName(), extension)
        Return oPath
    End Function
End Class
