...
Public Class CDeclaraciones
    ...
    Dim CAUSAR_LIQ As String
    Dim PD_NRODOC As String = ""
    Dim PD_TIPDOC As String = ""
    Dim PD_RAZSOC As String = ""
    'Datos de Revisor o Contador
    Dim RF_NRODOC As String = ""
    Dim RF_TIPDOC As String = ""
    Dim RF_RAZSOC As String = ""
    Dim RF_TREV As String = ""
    Dim RF_TARPRO As String = ""
    ...
    'me.querystring = "Select * from vLiqDec Where Cld_Cod=:CD "
    'me.querystring = "Select * from CONC_CDEC c where c.cocd_cdec=:cdec and  c.COCD_SECO=:SECO Order By COCD_CDEC,COCD_CODI"
    'me.querystring = "Select * from VCONC_CDEC c where c.cocd_fdco=:cocd_fdco and  c.COCD_SECO=:SECO And (Select cal_fini From Calendario Where Cal_Cla=c.cocd_cdec And  Cal_Vig=:Cal_Vig And Cal_Per=:Cal_Per) Between c.COCD_FEIN and c.COCD_FEFI Order By COCD_CDEC,COCD_CODI"
    'CrearComando(querystring)
    ' 
    '    dbCommand.Parameters.Add("cocd_fdco", OracleDbType.Varchar2, ParameterDirection.Input).Value = FODE_CODI
    '    dbCommand.Parameters.Add("Seco", OracleDbType.Varchar2, ParameterDirection.Input).Value = Sec_Cod
    '    dbCommand.Parameters.Add("Cal_Vig", OracleDbType.Varchar2, ParameterDirection.Input).Value = Año
    '    dbCommand.Parameters.Add("Cal_Per", OracleDbType.Varchar2, ParameterDirection.Input).Value = Periodo
    '     '
    'Dim dataTb As DataTable = EjecutarConsultaDatatable
    '    
    '  Me.Desconectar()
    ...
    Public Function GetTarifa(ByVal inCimp As String, ByVal inPar_Input As String, ByVal Fecha_Dec As Date) As Double
        ...
        'tar = DirectCast((pReturn.Value), OracleDecimal).ToDouble
        'tar = CDbl(preturn.Value.ToString.Replace(".", ","))
    ...
    Public Function Valor_Pagado(ByVal Dec_Cod As String) As Decimal
        ...
        Me.querystring = "Select code_vade from code_cdec where code_seco='VP' and code_tico='T' and code_dcod=:dec_cod"
        CrearComando(querystring)
        AsignarParametroCadena(":dec_cod", Dec_Cod)
    ...
    Public Function GetFormularios_Dec(ByVal Año As String, ByVal Periodo As String) As DataSet
        Me.Conectar()
        Me.querystring = "Select * From Formularios_Dec f Where f.fode_cdec=:CDec And (Select cal_fini From Calendario Where Cal_Cla=f.fode_cdec And  Cal_Vig=:Cal_Vig And Cal_Per=:Cal_Per) Between f.fode_FEIN and f.fode_FEFI Order By fode_CDEC,fode_CODI"
        CrearComando(querystring)
